...
int main() {
    ...
    for (int case_id : {1, 2, 3, 4, 5, 6, 7}) {
        ...
        // Case 1-4: Shock wave interact with rigid bodies (circle, square, two circles, two overlapping circles)
        ...
        // Case 5: Static circle interact with moving fluid; Case 6: Moving circle interact with initially static fluid
        ...
        phi.resize(nCellsX + 4, std::vector<double>(nCellsY + 4));

        // update data
        double t = tStart;
        double sim_period = tStop - tStart;
        std::array t_record_list = {sim_period * 0.25, sim_period * 0.5, sim_period * 0.75, sim_period * 1.0};
        int record_index = 0;
        int counter = 0;
        do {
            ...
            calLevelSet(phi, rigid_center, nCellsX, nCellsY, x0, y0, dx, dy, case_id);
            ...
            if (t >= t_record_list[record_index]) {
                ...
                folderPath << "D:/Study_Master/WrittenAssignment/WorkSpace/Rigid_Body/res/Case_" << case_id;
