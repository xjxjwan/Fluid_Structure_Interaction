...
void RiemannSolver::GetWaveTypeMode(double gama, double p_inf) {
    ...
    if (p_star > left_state[2] && p_star > right_state[2]) {
        ...
        std::cout << left_state[0] << " " << left_state[1] << " " << left_state[2] << std::endl;
        std::cout << right_state[0] << " " << right_state[1] << " " << right_state[2] << std::endl;
        assert(false);
    }
}
...
std::array<double, 3> RiemannSolver::SolveSinglePoint(double gama, double x) {
    ...
    if (wave_type_mode == 3) {
        if (cur_x - R_head_l * sim_time < 0) {u_i = std::array{rho_l, v_l, p_l};}
            ...
            double p_raref_l = p_r * pow(rho_raref_l / rho_l, gama);
    ...
    if (wave_type_mode == 5) {
        if (not (rho_l == rho_r && v_l == v_r && p_l == p_r)) {
            assert(false);
        }
